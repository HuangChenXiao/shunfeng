...
{
    public class U8_sAp_Voucah_Add
    {
        public bool U8_sAp_Voucah(Ap_Vouch main, List<Ap_Vouchs> detailList, out string resultmsg)
        {
            ...
            //设置上下文参数
            envContext.SetApiContext("VouchType", "应收单"); //上下文数据类型：string，含义：单据类型：应收单

            //第三步：设置API地址标识(Url)
            //当前API：新增或修改的地址标识为：U8API/ARVouch/SaveVouch
            U8ApiAddress myApiAddress = new U8ApiAddress("U8API/ARVouch/SaveVouch");
            ...
            //给BO表头参数ohead赋值，此BO参数的业务类型为应收单，属表头参数。BO参数均按引用传递
            ...
            ohead[0]["cCusAbbName"] = ""; //客户名称，string类型
            ohead[0]["cexch_name"] = main.cexch_name; //币种，string类型
            ohead[0]["iAmount_f"] = main.iAmount_f; //金额，double类型
            ohead[0]["cagentcusname"] = ""; //委托方名称，string类型
            ohead[0]["cagentcusabbname"] = ""; //委托方简称，string类型
            ohead[0]["cContractType"] = ""; //合同类型编码，string类型
            ohead[0]["cContractTypeName"] = ""; //合同类型名称，string类型
            ohead[0]["cContractID"] = ""; //合同号，string类型
            ohead[0]["cContractName"] = ""; //合同名称，string类型
            ohead[0]["cOrderNo"] = ""; //订单号，string类型
            ohead[0]["csysbarcode"] = ""; //单据条码，string类型
            ohead[0]["cCusName"] = ""; //客户全称，string类型
            ohead[0]["iSource"] = ""; //来源，string类型
            ohead[0]["iBusType"] = ""; //业务类型编号，string类型
            ohead[0]["cBusType"] = ""; //业务类型，string类型
            ohead[0]["cagentcuscode"] = ""; //委托方编码，string类型
            ...
            //给BO表体参数obody赋值，此BO参数的业务类型为应收单，属表体参数。BO参数均按引用传递
            ...
            obody[0]["cexch_name"] = item.cexch_name; //币种，string类型
            ...
            //获取返回值
            //获取普通返回值。此返回值数据类型为System.Boolean，此参数按值传递，表示返回值：true表示成功，false表示失败。
